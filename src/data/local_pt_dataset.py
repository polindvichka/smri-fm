...
def crop_or_pad(
    image: Tensor, crop_size: tuple[int, int, int], train: bool, force_random: bool = False
) -> tuple[Tensor, Tensor]:
...
class LocalPtDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        for attempt in range(8):
            image, valid = crop_or_pad(
                image_full, self.crop_size, self.train, force_random=attempt > 0
            )
...
def split_train_val(
    paths: list[str], val_fraction: float, seed: int = 0
